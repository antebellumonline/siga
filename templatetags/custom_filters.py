...
def render_search_fields(fields, request):
    ...
    for field in fields:
        ...
        if field_type == "select":
            form_html += f'<select name="{field_name}" id="{field_name}" class="select-search select2">'
            ...
            form_html += f'<input type="{field_type}" id="{field_name}" name="{field_name}" value="{value}" class="form-control">'
